...
````yaml /api-reference/oapi-schemas/sports-schema.json get /v2/leagues/{slug}/events
openapi: 3.0.3
info:
...
paths:
  /v2/leagues/{slug}/events:
    get:
      tags:
        - Sports
      summary: Get Events By League Slug
      description: >-
        Retrieve events for a league by its slug (e.g., 'nfl', 'nba'). Resolves
        the league to its active series and returns events.
      operationId: SportsV2Service_GetLeagueEvents
      parameters:
        - name: slug
          description: League slug (e.g., 'nfl', 'nba')
          in: path
          required: true
          schema:
            type: string
      ...
      responses:
        '200':
          description: League events with league info
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/gateway.sports.v2.GetLeagueEventsResponse'
        '500':
...
components:
  schemas:
    gateway.sports.v2.GetLeagueEventsResponse:
      type: object
      properties:
        type:
          type: string
      ...
      description: Response containing events for a league
